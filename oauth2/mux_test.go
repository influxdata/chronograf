...
// should be deferred.
func setupMuxTest(selector func(*CookieMux) http.Handler) (*http.Client, *httptest.Server, *httptest.Server) {
	...
	jm := NewCookieMux(mp, auth, mt, clog.New(clog.ParseLevel("debug")))
...
func Test_CookieMux_Logout_DeletesSessionCookie(t *testing.T) {
	...
	hc, ts, prov := setupMuxTest(func(j *CookieMux) http.Handler {
		return j.Logout()
	})
