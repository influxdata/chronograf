...
export const functions: FluxToolbarFunction[] = [
	...
	},
	{
 		...
 		desc: 'This is a special application of the `pivot()` function that will automatically align fields within each measurement that have the same timestamp.',
	...
	},
	{
  		...
  		args: [
				{
    			name: 'by',
    			desc: 'List of columns by which to group. Cannot be used with `except`.',
    			type: 'Array of Strings'
    		},
				{
    			name: 'except',
    			desc: 'List of columns by which to NOT group. All other columns are used to group records. Cannot be used with `by`.',
    			type: 'Array of Strings'
    		},
				{
    			name: 'none',
	...
	},
	{
  		...
  		args: [
				...
				{
    			name: 'buckets',
    			desc: 'A list of upper bounds to use when computing the histogram frequencies. Buckets should contain a bucket whose bound is the maximum value of the data set. This value can be set to positive infinity if no maximum is known.',
 		...
 		example: 'histogram(column: "_value", upperBoundColumn: "le", countColumn: "_value", buckets: [50.0, 75.0, 90.0], normalize: false)',
 		category: 'Transformation',
 		link: 'http://example.com'
	},
	{
 		...
 		example: 'histogramQuantile(quantile: 0.5, countColumn: "_value", upperBoundColumn: "le", valueColumn: "_value", minValue: 0)',
 		category: 'Transformation',
 		link: 'http://example.com'
	},
	{
	...
	},
	{
  		...
  		args: [
				{
    			name: 'every',
    			desc: 'The duration between starts of each of the intervals. Defaults to the value of the `period` duration.',
    			type: 'Duration'
    		},
				{
    			name: 'period',
    			desc: 'The length of each interval. Defaults to the value of the `every` duration.',
    			type: 'Duration'
    		},
				{
    			name: 'offset',
    			desc: 'The offset duration relative to the location offset. Defaults to `0h`.',
    			type: 'Duration'
    		},
				{
    			name: 'columns',
				...
				{
    			name: 'fn',
    			desc: 'A function that accepts an interval object and returns a boolean value. Each potential interval is passed to the filter function. When the function returns false, that interval is excluded from the set of intervals. Defaults to include all intervals.',
    			type: 'Function'
    		}
 		],
