...
import {showOverlay as showOverlayAction} from 'shared/actions/overlay'
import Authorized, {EDITOR_ROLE} from 'src/auth/Authorized'

import Dropdown from 'shared/components/Dropdown'
import QuestionMarkTooltip from 'shared/components/QuestionMarkTooltip'
import SourceOverlay from 'src/sources/components/SourceOverlay'
...
const InfluxTable = ({
  ...
  const handleAddSource = () => {
    showOverlay(<SourceOverlay />)
  }

  const handleEditSource = sourceID => () => {
    showOverlay(<SourceOverlay sourceID={sourceID} editMode={true} />)
  }
                      ...
                      <td>
                        <h5 className="margin-zero">
                          ...
                          >
                            <a
                              href="#"
                              className={s.id === source.id && 'link-success'}
                              onClick={handleEditSource(s.id)}
                            >
                              <strong>
                                {s.name}
                              </strong>
...
InfluxTable.propTypes = {
  ...
  isUsingAuth: bool,
  showOverlay: func.isRequired,
}

const mapStateToProps = ({auth: {isUsingAuth, me}}) => ({isUsingAuth, me})

const mapDispatchToProps = dispatch => ({
  showOverlay: bindActionCreators(showOverlayAction, dispatch),
})

export default connect(mapStateToProps, mapDispatchToProps)(
  withRouter(InfluxTable)
)
