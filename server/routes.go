...
type getRoutesResponse struct {
	...
	DashboardServer string      `json:"dashboardd,omitempty"` // Locaiton of the dashboards gallery server
	Auth            []AuthRoute `json:"auth"`                 // Location of all auth routes.
}

// AllRoutes returns all top level routes within chronograf
func AllRoutes(authRoutes []AuthRoute, dashboardServer string, logger chronograf.Logger) http.HandlerFunc {
	...
	if dashboardServer != "" {
		routes.DashboardServer = dashboardServer
	}
